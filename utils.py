...
from constants import users, genres, greetings, page_size
...
async def send_recommendation(query, bot, kino_resp, counter):
    if kino_resp["total"] == 0:
        bot.send_message(query.from_user.id, 'Дурацкий конопоиск ничего не нашел(((')
    else:
